...
DiskHealth disk_compute_health(const Disk& d) {
    ...
    if (d.device_status != 0) return DiskHealth::FAIL;
    if (bad_sectors)          return DiskHealth::WARN;
...
int disks_snapshot(Disk* out, int max) {
    ...
    for (int i = 0; i < n; ++i) out[i] = s_disks[i];
    unlock();
    return n;
}

void disks_note_fetch_ok()   { lock(); s_consec_fail = 0; unlock(); }
void disks_note_fetch_fail() { lock(); ++s_consec_fail; unlock(); }

int disks_consec_fail() { lock(); int v = s_consec_fail; unlock(); return v; }
int disks_count()       { lock(); int v = s_count; unlock(); return v; }

int64_t disks_last_fetch_sec() { lock(); int64_t v = s_last_fetch_sec; unlock(); return v; }
