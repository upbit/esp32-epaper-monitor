...
static const char* esc(const char* s, char* dst, size_t cap) {
    ...
    for (size_t i = 0; s && s[i] && j + 7 < cap; ++i) {
        char c = s[i];
        if      (c == '<') { memcpy(dst + j, "&lt;", 4);  j += 4; }
        else if (c == '>') { memcpy(dst + j, "&gt;", 4);  j += 4; }
        else if (c == '&') { memcpy(dst + j, "&amp;", 5); j += 5; }
        else dst[j++] = c;
    }
    dst[j] = '\0';
    return dst;
...
static const char* health_str(DiskHealth h) {
    switch (h) { case DiskHealth::OK: return "OK"; case DiskHealth::WARN: return "WARN"; default: return "FAIL"; }
}
...
static esp_err_t h_root(httpd_req_t* req) {
    ...
    if (fmin < 0) chunkf(req, "Data: <span class=stale>not fetched yet</span>");
    else          chunkf(req, "Last update: %dm ago &nbsp; Disks: %d", fmin, n);
    if (stale) chunkf(req, " &nbsp; <span class=stale>STALE (%d fails)</span>", fail);
    ...
    if (n == 0) {
        ...
        char m[64], s[48], nm[24];
        for (int i = 0; i < n; ++i) {
            const Disk& d = disks[i];
            int tw = d.temp; if (tw < 0) tw = 0; if (tw > 60) tw = 60;
