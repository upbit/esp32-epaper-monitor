...
{
    if (px < 0 || px >= Ssd1675a213::WIDTH_VISIBLE || py < 0 || py >= Ssd1675a213::HEIGHT)
        return;
    const int idx = (px >> 3) + py * Ssd1675a213::ROW_BYTES;
    const uint8_t mask = 0x80 >> (px & 7);
    if (c == EPD_WHITE)
        _buf[idx] |= mask; // 1 = white
    else
        _buf[idx] &= ~mask; // 0 = black (red -> black in mono)
}
    ...
    for (int col = 0; col < 5; ++col)
    {
        ...
        for (int row = 0; row < 7; ++row)
        {
            ...
            if (!on && !bg_on)
                continue;
            if (size == 1)
                drawPixel(x + col, y + row, pc);
            else
                fillRect(x + col * size, y + row * size, size, size, pc);
        }
    }
    // 6th spacing column.
    if (bg_on)
