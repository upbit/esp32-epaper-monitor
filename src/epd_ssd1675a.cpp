...
void Ssd1675a213::_initDisplay() {
    ...
    _cmd(0x74); _data(0x54);            // set analog block control
    _cmd(0x7E); _data(0x3B);            // set digital block control
    _cmd(0x01); _data(0xF9); _data(0x00); _data(0x00); // driver output control (HEIGHT-1=249)
    _cmd(0x3C); _data(0x03);            // border waveform
    _cmd(0x2C); _data(0x70);            // VCOM
    _cmd(0x03); _data(0x15);            // gate driving voltage (19V)
    _cmd(0x04); _data(0x41); _data(0xA8); _data(0x32); // source driving voltage
    _cmd(0x3A); _data(0x30);            // dummy line
    _cmd(0x3B); _data(0x0A);            // gate time
    _setRamAreaFull();
}

void Ssd1675a213::_setRamAreaFull() {
    _cmd(0x11); _data(0x03);            // data entry: X inc, Y inc
    _cmd(0x44); _data(0x00); _data((RAM_WIDTH - 1) / 8); // RAM-X start/end (0..15)
    _cmd(0x45);                          // RAM-Y start/end (0..249)
    _data(0x00); _data(0x00);
    _data((HEIGHT - 1) % 256); _data((HEIGHT - 1) / 256);
    _cmd(0x4E); _data(0x00);            // RAM-X counter
    _cmd(0x4F); _data(0x00); _data(0x00); // RAM-Y counter
}
...
void Ssd1675a213::_initPart() {
    _initDisplay();
    _cmd(0x2C); _data(0x26);            // VCOM override for partial
    _cmd(0x32);
...
void Ssd1675a213::hibernate() {
    _powerOff();
    _cmd(0x10); _data(0x01); // deep sleep mode
    _hibernating = true;
